...
export function NewsSection({ items = mockNewsItems }: NewsSectionProps) {
  return (
    <section id="news" className="relative py-16 sm:py-20 lg:py-24">
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mt-10 sm:mt-16 grid gap-5 sm:gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          <RevealList interval={0.15}>
            {items.map((item) => (
              <article
                key={item.id}
                className="group relative overflow-hidden h-full rounded-2xl border border-slate-200/70 bg-white shadow-[0_20px_44px_-28px_rgba(15,23,42,0.18)] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_32px_64px_-32px_rgba(15,23,42,0.25)]"
              >
                <div className="relative h-44 sm:h-48 overflow-hidden">
                  ...
                  <div className="absolute top-3 sm:top-4 left-3 sm:left-4">
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-white/90 backdrop-blur-sm px-2.5 py-1 text-[10px] sm:text-xs font-semibold text-slate-700">
                      <NewspaperIcon className="h-3 w-3" />
                      {item.category}
                    </span>
                  </div>
                  <div className="absolute bottom-3 sm:bottom-4 left-3 sm:left-4 right-3 sm:right-4">
                    <div className="flex items-center gap-1.5 text-[10px] sm:text-xs font-medium text-white/90">
                ...
                <div className="p-4 sm:p-6">
                  <h3 className="mb-2 sm:mb-3 text-base sm:text-xl font-bold text-slate-900 group-hover:text-primary transition-colors duration-300 leading-snug">
                    {item.title}
                  </h3>
                  <p className="mb-3 sm:mb-4 text-xs sm:text-sm text-slate-600 leading-relaxed line-clamp-3">
                    {item.excerpt}
                  </p>
                  {item.slug && (
                    <Link
                      href={`/news/${item.slug}`}
                      className="inline-flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm font-semibold text-primary hover:text-primary-light transition-colors duration-300 group-hover:gap-2.5"
                    >
                      Read More
                      <ArrowRightIcon className="h-3.5 w-3.5 sm:h-4 sm:w-4 transition-transform duration-300 group-hover:translate-x-1" />
                    </Link>
                  )}
                </div>
              </article>
        ...
        <Reveal y={10} delay={0.4}>
          <div className="mt-10 sm:mt-12 text-center">
            <Link
              href="/news"
              className="inline-flex items-center gap-2 sm:gap-3 rounded-full border-2 border-primary/20 bg-primary/5 px-6 sm:px-8 py-3 text-xs sm:text-sm font-bold uppercase tracking-[0.2em] sm:tracking-[0.28em] text-primary transition-all duration-300 hover:border-primary hover:bg-primary hover:text-white hover:shadow-lg hover:shadow-primary/20"
            >
              View All News
