...
export function SectionHeading({ id, eyebrow, title, subtitle, tone = "dark" }: SectionHeadingProps) {
  ...
  return (
    <header id={id} className={`mx-auto max-w-3xl text-center ${tone === "light" ? "text-slate-700" : "text-slate-200"}`}>
      {eyebrow ? (
        <span className={`text-[13px] font-bold uppercase tracking-[0.28em] ${baseEyebrow}`}>
          {eyebrow}
        </span>
      ) : null}
      <h2 className={`mt-3 text-3xl font-light leading-tight sm:text-[2.2rem] ${titleColor}`}>{title}</h2>
